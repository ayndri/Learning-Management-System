...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        // --- LOGIKA LOGIN STATIS ---
        // Simulasi delay network (biar terasa seperti loading beneran)
    ...
    // Fungsi dummy untuk tombol sosmed
    const handleSocialLogin = () => {
        alert("Ini hanya demo statis. Silakan login menggunakan Email & Password di atas.");
    };

    return (
        <div className="min-h-screen w-full flex bg-white">
            {/* Bagian Kiri (Artistik) */}
            <div className="hidden lg:flex w-1/2 bg-indigo-900 relative items-center justify-center overflow-hidden">
                ...
                <div className="relative z-20 text-white p-12 max-w-lg">
                    <h2 className="text-5xl font-extrabold mb-6 leading-tight">Welcome Back!</h2>
                    <p className="text-indigo-200 text-lg">Silakan login untuk melanjutkan progress belajarmu.</p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-16 overflow-y-auto">
                <div className="w-full max-w-md space-y-8">
                    <div>
                        ...
                        <p className="text-gray-500 mt-2">Gunakan akun demo untuk masuk.</p>
                    </div>

                    {/* Alert Error */}
                    {error && (
                        <div className="bg-red-50 text-red-600 p-3 rounded-lg text-sm border border-red-200 animate-pulse">
                            ⚠️ {error}
                        </div>
                    )}
                    ...
                    <form className="mt-8 space-y-6" onSubmit={handleLogin}>
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full flex justify-center py-3 px-4 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors disabled:bg-indigo-400 shadow-lg shadow-indigo-200"
                        >
                            {isLoading ? (
                    ...
                    <div className="bg-yellow-50 p-4 rounded-lg text-xs text-yellow-800 border border-yellow-200 mt-4">
